...
const Home = () => {
  ...
  return (
    <div ref={ref} id="home" className=" h-screen sm:h-screen w-full z-10">
      {/* <MovingCircle />
			<style>
				{`
          @keyframes move-up {
      ...
      <div className="max-w-screen-lg mx-auto flex flex-col items-center h-full px-4 md:flex-row">
        <div className="flex flex-col py-20 sm:py-0 justify-center items-center w-full h-full">
          <h1
            className={`text-6xl flex flex-col -mt-10 gap-2 sm:gap-6 sm:flex-row items-center md:text-8xl font-bold pb-2 transition duration-[1500ms] ${
              isVisible
          ...
          >
            <Writer />
                  ...
                  <p
        className={` text-center mt-10 px-5  text-base md:text-lg text-gray-600 dark:text-gray-300 pb-4 transition duration-[1000ms] ${
          isVisible
            ? "opacity-100 translate-x-0"
            : "opacity-0 translate-x-5 sm:translate-x-20"
        }`}
      >
        Full-stack developer building fast, scalable web apps with Vue, Node.js, and MongoDB.
      </p>
          <div className="flex gap-4">
            <a
            href="https://drive.google.com/file/d/1mwlEyiWVl0JG14657coQiPCBMu5Yw85H/view?usp=sharing"
            target="_blank"
            rel="noopener noreferrer"
            className={`group rounded-md w-fit px-6 py-3 my-2 flex items-center font-extrabold cursor-pointer transition duration-[300ms] hover:text-[#FFD369] ${
              isVisible
                ? "opacity-100 translate-x-0"
          ...
          >
           ...
           <Link
            to="contact"
            className={`group rounded-md w-fit px-6 py-3 my-2 flex items-center font-extrabold cursor-pointer transition duration-[300ms] text-blue-600 ${
              isVisible
                ? "opacity-100 translate-x-0"
                : "opacity-0 translate-y-5 sm:translate-y-20"
            }`}
